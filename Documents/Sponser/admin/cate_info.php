...
$made_id=make_id('cate_info','cate_id','Cate--','8');
...
$select_db=mysql_query("SELECT * FROM cate_info");
...
print "$id_name<br><br><br>";
	$search_db=mysql_query("SELECT * FROM cate_info WHERE cate_id LIKE '%$id_name%' OR cate_name LIKE '%$id_name%'");
...
if(isset($_POST['add']))
{
	...
	if(!empty($cate_id) && !empty($cate_name) && !empty($itm_slct))
	{
		mysql_query("INSERT INTO cate_info(`item_name`,`cate_id`,`cate_name`)VALUE('$itm_slct','$made_id','$cate_name')");
...
<h2>Category Information</h2>
	...
	<div class="items">
		<div class="search">
			<input type="text" name="id_name" placeholder="Category ID or Name">
			<div>
		...
		<div>
			...
			<input type="text" name="cate_id" value="<?php print $made_id ?>">
		</div>
		<div>
